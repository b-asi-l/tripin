...
export const ReceiptScreen: React.FC<Props> = ({ booking, onHome, onTrack, onChat, onCancelSuccess }) => {
  ...
  const handleCopyPhone = () => {
    if (booking.ownerPhone) {
      navigator.clipboard.writeText(booking.ownerPhone);
      setCopied(true);
    ...
    <div className="p-8 h-full flex flex-col items-center justify-center animate-in zoom-in-95 duration-500">
      <div className={`bg-surface p-8 rounded-[48px] border border-subtle card-shadow w-full text-center space-y-6 relative overflow-hidden ${isCancelled ? 'opacity-80' : ''}`}>
        ...
        <div className="border-t border-b border-dashed border-subtle py-6 space-y-4">
            ...
            <div className="flex justify-between text-xs items-center">
                <span className="text-muted font-bold uppercase">Phone</span>
                <div className="flex items-center gap-2">
                    <span className="text-main font-black select-all">{booking.ownerPhone || 'N/A'}</span>
                    {booking.ownerPhone && (
                        <button 
                            onClick={handleCopyPhone}
                            className="text-emerald-500 hover:text-emerald-700 active:scale-90 transition-transform p-1"
                            title="Copy Phone Number"
                        >
                            {copied ? (
                                <span className="text-[8px] font-black uppercase bg-emerald-100 px-1 rounded">Copied</span>
                            ) : (
            ...
            <div className="flex justify-between text-xs items-center">
             ...
             <div className="flex justify-between text-lg pt-2">
                ...
                <span className={`font-black ${isCancelled ? 'text-muted line-through' : 'text-[#16A34A]'}`}>₹{booking.amount + 5}</span>
            </div>
        </div>

        {!isCancelled && (
            <div className="space-y-3">
                <button onClick={onTrack} className="w-full bg-[#16A34A] text-white py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-500/20 active:scale-95 transition-all flex items-center justify-center gap-3">
                    <Icons.Car className="w-4 h-4" /> Track Live Ride
                </button>
                
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={onHome} className="w-full bg-surface-alt text-main py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-subtle active:scale-95">Home</button>
                  <button onClick={onChat} className="w-full bg-surface-alt text-main py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-subtle active:scale-95">Chat</button>
                </div>
